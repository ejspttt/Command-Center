...
function updateScoreForStudent(studentName) {
  ...
  const studentRowIndex = studentNames.findIndex(row => row[0] === studentName) + 1;
  ...
  if (studentSheet) {
    ...
    if (data.length > 0) {
      ...
      const averageColIndex = headers.findIndex(header => header.toString().trim() === "Overall Average");
      const totalRowIndex = data.findIndex(row => row[0].toString().includes("Weekly Total:"));
...
function createSheetsFromClassroom() {
  ...
  if (students.length === 0) {
    ui.alert('No students found in the specified course. Please check the Course ID.');
    return;
  }
  ...
  const studentNamesForSheet = students.map(student => [student.profile.name.fullName]);
  const targetSheet = spreadsheet.getSheetByName('Command Center') || spreadsheet.getSheets()[0];
  
  targetSheet.getRange(2, 1, targetSheet.getMaxRows(), 1).clearContent();
  targetSheet.getRange(2, 1, studentNamesForSheet.length, 1).setValues(studentNamesForSheet);
  
  const summaryMessage = `Process Complete. New sheets created: ${createdCount}.\n\n` +
...
function createCommandCenter() {
  ...
  try {
    ...
    headerRange.setValues(headers).setFontWeight('bold').setBackground('#d9ead3');

    const studentListRange = commandCenterSheet.getRange(2, 1, commandCenterSheet.getLastRow() - 1, 1);
    const studentNames = studentListRange.getValues().map(row => row[0]).filter(String);

    if (studentNames.length === 0) {
      ui.alert('No student names found in Column A. Please run "1. Create Student Sheets & Update List" first.');
      return;
    }
    ...
    ui.alert('Success!', 'Command Center has been set up and all student scores have been populated.', ui.ButtonSet.OK);

  } catch (e) {
    Logger.log('Error processing Command Center: ' + e.message);
    ui.alert('An error occurred while processing the Command Center. Please check the logs for details.');
  }
}
...
function setupRubric() {
  ...
  const ui = SpreadsheetApp.getUi();
  const response = ui.prompt('Setup Rubric', 'Enter the total number of weeks for the rubric:', ui.ButtonSet.OK_CANCEL);

  if (response.getSelectedButton() != ui.Button.OK) return;

  const desiredWeeks = parseInt(response.getResponseText(), 10);
  if (isNaN(desiredWeeks) || desiredWeeks < 0) {
    ui.alert('Invalid Input', 'Please enter a valid, non-negative number.', ui.ButtonSet.OK);
    return;
  }

  try {
    if (sheet.getLastRow() === 0) {
      const defaultCategories = [['Category'], ['Professionalism'], ['Safety'], ['Clean Up'], ['Participation'], ['Weekly Total:']];
      sheet.getRange(1, 1, defaultCategories.length, 1).setValues(defaultCategories);
    }

    const columnAValues = sheet.getRange("A:A").getValues();
    let totalRow = columnAValues.findIndex(row => row[0].toString().includes("Weekly Total:")) + 1;

    if (totalRow === 0) {
      ui.alert('Error', "Could not find the 'Weekly Total:' row. Please ensure this text exists in Column A.", ui.ButtonSet.OK);
      return;
    }
    ...
    if (startDataRow >= endDataRow) {
      ui.alert('Error', "No categories found. Please add category names above the 'Weekly Total:' row.", ui.ButtonSet.OK);
      return;
    }
    ...
    for (let week = 1; week <= desiredWeeks; week++) {
      ...
      const formula = `=SUM(${sheet.getRange(startDataRow, scoreColumn).getA1Notation()}:${sheet.getRange(endDataRow, scoreColumn).getA1Notation()})`;
      sheet.getRange(totalRow, scoreColumn).setFormula(formula);
      weeklyTotalCells.push(sheet.getRange(totalRow, scoreColumn).getA1Notation());
    }
    ...
    ui.alert('Success!', `The rubric has been set up for ${desiredWeeks} weeks.`, ui.ButtonSet.OK);

  } catch (e) {
    ui.alert('Error', 'Could not set up the rubric. Error: ' + e.toString(), ui.ButtonSet.OK);
  }
}
...
function applyRubricFormatting(sheet, totalRow, endDataRow, startDataRow) {
  ...
  sheet.getRange(1, 1, 1, lastCol).setBackground(headerColor).setFontWeight('bold');
  sheet.getRange(totalRow, 1, 1, lastCol).setBackground(totalColor).setFontWeight('bold');
  ...
  const scoreRange = sheet.getRange(startDataRow, 2, endDataRow - startDataRow + 1, lastCol - 2);
...
function selectAndSetCourseId() {
  try {
    ...
    if (!courses || courses.length === 0) {
      ui.alert('No Courses Found', 'No courses were found for your account.\n\nPlease ensure:\n1. The Classroom API is enabled for this script.\n2. You have authorized the script with the correct Google account.', ui.ButtonSet.OK);
      return;
    }
    ...
    if (err.message.includes("Classroom is not defined")) {
       ui.alert('Error: The Classroom API is not enabled.', 'Please go to the Script Editor, click on "Services +", find "Google Classroom API", and click "Add".', ui.ButtonSet.OK);
    } else {
       ui.alert('Failed to fetch courses. Please check your permissions. Error: ' + err.message);
    }
  }
}
...
function saveSelectedCourseId(courseId) {
  if (courseId) {
    ...
    ui.alert('Success', `The active Course ID has been set to: ${courseId}`, ui.ButtonSet.OK);
  }
}
...
function getStudentRoster(courseId) {
  try {
    ...
    if (e.message.includes("Classroom is not defined")) {
       SpreadsheetApp.getUi().alert('Error: The Classroom API is not enabled.', 'Please go to the Script Editor, click on "Services +", find "Google Classroom API", and click "Add".', SpreadsheetApp.getUi().ButtonSet.OK);
    } else {
       SpreadsheetApp.getUi().alert('An error occurred while fetching the student roster. Please ensure your Course ID is correct and that you have permission to access it. Check logs for details.');
    }
    return null;
  }
}
